...
//        this.newMsgPeers.clear();
     ...
     * @return 缺失的消息集合
     */
    private List<Message> getMissingMessage1(List<Message> messageList, byte[] hashPrefixArray) {
        List<Message> missingMessage = new ArrayList<>();
    ...
    private class PositionInfo {
        public int position;
        public boolean nonRepeated = false;
     ...
     * @return confirmation root集合
     */
    private List<byte[]> getConfirmationRoot1(List<Message> messageList, byte[] hashPrefixArray) {
        List<byte[]> confirmationRootList = new ArrayList<>();
...
//            byte[] nonRepeatedArray = new byte[0];
            int size = messageList.size();
            int length = hashPrefixArray.length;

            List<PositionInfo> positiveOrderList = new ArrayList<>();
            List<PositionInfo> negativeOrderList = new ArrayList<>();
            for (int i = 0; i < length; i++) {
                positiveOrderList.add(new PositionInfo());
                negativeOrderList.add(new PositionInfo());
            }

            for (int i = 0; i < length; i++) {
                boolean found = false;
                for (int j = 0; j < length; j++) {
                    if (hashPrefixArray[i] == hashPrefixArray[j] && i != j) {
                        found = true;
                        break;
                    }
                }
                ...
                if (!found) {
//                    nonRepeatedArray = ByteUtil.appendByte(nonRepeatedArray, hashPrefixArray[i]);
                    PositionInfo positionInfo = positiveOrderList.get(i);
                    positionInfo.nonRepeated = true;
                    positiveOrderList.set(i, positionInfo);

                    positionInfo = negativeOrderList.get(i);
                    positionInfo.nonRepeated = true;
                    negativeOrderList.set(i, positionInfo);
                }
            }

//            for(byte prefix: nonRepeatedArray) {
            for(int index = 0; index < length; index++) {
                if (positiveOrderList.get(index).nonRepeated) {
                    byte prefix = hashPrefixArray[index];
                    ...
                    if (1 == matchTimes) {
                        ...
                        if (1 == current) {
                            PositionInfo positionInfo = positiveOrderList.get(index);
                            positionInfo.matched = true;
                            positionInfo.position = position;
                            positiveOrderList.set(index, positionInfo);
                        } else if (-1 == current) {
                            PositionInfo positionInfo = negativeOrderList.get(index);
                            positionInfo.matched = true;
                            positionInfo.position = position;
                            negativeOrderList.set(index, positionInfo);
                        }
...
//            }
            ...
            if (sum >= 0) {
                ...
                do {
                    ...
                    for (int i = 0; i < length; i++) {
                        // 往左找
                        if (!positiveOrderList.get(i).matched && positiveOrderList.get(i + 1).matched) {
                            // 左边界
                            int leftBoundary = 0;
                        }
                        // 往右找

                        if (i < length - 1 && positiveOrderList.get(i).matched && !positiveOrderList.get(i + 1).matched) {
                            int rightBoundary = size;
                            for (int m = i + 1; m < length; m++) {
                                if (positiveOrderList.get(m).matched) {
                                    rightBoundary = positiveOrderList.get(m).position;
                                    break;
                                }
                            }

                            for (int j = positiveOrderList.get(i).position + 1; j < rightBoundary; j++) {
                                Message message = messageList.get(j);
                                byte[] hash = message.getHash();
                                if (hash[0] == hashPrefixArray[i + 1]) {
                                    go = true;
                                    break;
                                }
                                if (positiveOrderList.get(j).matched) {
                                    break;
                                }
                            }
                        }
...
//                for (Integer index: negativeOrderList) {
//                    list.set(index, true);
//                }
            }
     ...
     * @return confirmation root集合
     */
    private List<byte[]> getConfirmationRoot2(List<Message> messageList, byte[] hashPrefixArray) {
        List<byte[]> confirmationRootList = new ArrayList<>();
